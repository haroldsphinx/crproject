...
//console.log(temp.team_2);
        ...
        var insert = {
            ...
            team_1_text: temp.team_1_text,
            team_2_text: temp.team_2_text,
            team_1_over: temp.team_1_over,
            team_2_over: temp.team_2_over,
            max_ball: temp.max_ball,
            match_comment: temp.match_comment,
...
exports.getMatchDetailAPI = function (params, fn) {
...
exports.getCricStartedMatchAPI = function (params, fn) {
//    var id = parseInt(params.id);
    // var o_id = new ObjectID(params.id);
//    if (status == 'started' || status == 'notstarted') {
//        var order = 1; //ASC
...
//    }
//console.log(unique_id);
//    db.get().collection('cricapi_matches').aggregate([
//        {"$cricapi_matches": {"matchStarted": true}},
//        {$sort: {dateTimeGMT: -1}}
//    ]).toArray(
//            function (err, rows) {
//                if (err) {
...
//            });
     ...
     db.get().collection('cricapi_matches').find({"matchStarted": true, "type": { $in: ["Twenty20", "T20I", "ODI"]}}).project({fantasySummary: 0}).limit(20).sort({date: -1}).toArray(
            function (err, rows) {
                if (err) {
                    return fn(true, err);
                }
