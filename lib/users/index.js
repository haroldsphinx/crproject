...
//            curr_search: qsearch,
    ...
    // console.log(req);
    res.render('add_users.ejs', params);
    //});
...
//        {
//            rows[0].startdate = dateFormat(rows[0].startdate, 'yyyy-mm-dd');
//            rows[0].enddate = dateFormat(rows[0].enddate, 'yyyy-mm-dd');
//            if (err)
...
//    });
    ...
    process.getUserProfile(req, function (err, rows) {
        //console.log(rows);
        rows[0].startdate = dateFormat(rows[0].startdate, 'yyyy-mm-dd');
        rows[0].enddate = dateFormat(rows[0].enddate, 'yyyy-mm-dd');
        if (err)
...
app.post('/users/save-user', function (req, res) {
    ...
    req.body.startdate = dateFormat(req.body.startdate, 'yyyy-mm-dd');
    req.body.enddate = dateFormat(req.body.enddate, 'yyyy-mm-dd');
...
app.get('/users/delete-user/:id', function (req, res) {
